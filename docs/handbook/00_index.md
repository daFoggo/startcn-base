...
| 4 | `04_tanstack_start_query_router.md` | TanStack Start, Router, Query, SSR data rules + both data source patterns (Supabase & HTTP backend via ky) |
...
- Two data sources, both accessed via feature `server.ts` + `functions.ts`: the HTTP backend through the shared `ky` instance (`src/lib/ky.ts`) and Supabase through the shared client (`src/utils/supabase.ts`). Neither client is called from `queries.ts` or components.
- Auth uses the HTTP backend: tokens live in a server session cookie, `ky` attaches the Bearer header and auto-refreshes on 401.
