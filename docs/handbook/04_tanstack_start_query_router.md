...
description: Apply the canonical TanStack Start/Router/Query data-fetching patterns for both data sources (Supabase and the HTTP backend via ky). Use when writing or reviewing query functions, query keys/options, route loaders, mutations, or Supabase/ky access. Covers the query contract, loader policy, suspense rules, and QueryClient lifecycle.
---
...
- Adding a mutation, invalidating caches, or accessing a data source (Supabase or the HTTP backend via ky).
...
- Two data sources, both accessed server-side through feature `server.ts` + `functions.ts`:
  ...
  - **HTTP backend** via the shared `ky` instance (`src/lib/ky.ts`).
- Zod for validation.
...
### HTTP Backend (ky) Pattern

The FastAPI backend (base `http://localhost:40723`, prefix `/api/v1`) is accessed with the shared `ky` instance from `src/lib/ky.ts` — never hand-rolled `fetch` wrappers.
